...
pub fn a_body_that_lands_repaints_the_pane_waiting_for_it_and_no_other() {
    crate::gtk_case(async {
        ...
        let (account, shown, other) = {
            ...
            let other = repository.create(&mut other).await.expect("a message");
            ...
            shown.attachments = parsed
                ...
                .collect();
            let shown = repository.create(&mut shown).await.expect("a message");
        ...
        assert!(
            settle_until(async || window.reader().absent() == Some(Absent::Partial)).await,
        ...
        store_body(&database, shown, "See the attached figures.").await;
        wired.feeds.apply(&Event::BodyLoaded {
            account,
            message: other,
        });
        assert!(
            settle_while(async || window.reader().absent() == Some(Absent::Partial)).await,
        ...
        assert!(
            settle_until(async || window.reader().absent().is_none()).await,
        ...
        assert!(settle_while(async || window.reader().absent().is_none()).await);
        ...
        assert!(
            settle_until(async || panel.cursor().is_some_and(|node| node.downloaded)).await,
