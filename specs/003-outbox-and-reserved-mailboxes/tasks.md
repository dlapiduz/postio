...
- [X] T053 [P] [US1] **Covered** by `row::tests::a_scheduled_send_says_when_and_an_immediate_one_says_only_that_it_waits` and `storage_suite::drafts::a_scheduled_send_carries_its_due_time_and_an_immediate_one_does_not`
...
- [X] T069b [US1] A scheduled send shows its due time, not just "Waiting to send" (FR-007). The time is `operation_queue.next_attempt_at`, not on the draft or its message row, so the list has nothing to draw it from — carrying it to `Row` is a wider change than the word was
