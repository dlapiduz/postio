...
> **75 of 77 done. The two that are not are one thing waiting.** T048 is
> somebody pressing `c` and looking at the composer in both schemes; T072's
> by-eye half is the same check. T073 — the merge — happened on 2026-09-11
> ahead of them, on the maintainer's instruction; T048 records what was
> measured in place of the looking. Every WebKit test in this repository runs
> the software path (`WEBKIT_DISABLE_DMABUF_RENDERER=1`, the mitigation for
> #272), so none of the 4,095 passing tests has seen how the editor actually
> paints (#1307), which is why the looking is a task at all.
...
- [ ] T048 [US4] **Half of it is automated now, and half is still a person.** `scripts/appearance.sh` renders the composer through the paths somebody drives — a row clicked, the reader loading from the blob store, `e` pressed, the quote built by `quote_of` — and then reads the pixels back, so the colour claims have a verdict rather than an impression. Both schemes: the editing surface and the reader's message body are both exactly `--r-ground` (245,245,248 light, 43,43,45 dark), which is FR-073 and FR-074 measured. It is deliberately not a test and not in `check.sh`: it needs a compositor, and what is left is looking. Look at it: `cargo run -p postio-app`, press `c`, compare with a message body, switch schemes with a draft open. No test here runs on the accelerated path (#1307), so this step is the evidence. **Half of the looking is done, and not by looking: FR-074's dark value is measured.** A live run on 2026-09-11 was screenshotted and the pixels read off it — the message body's interior and the pane around it are both exactly `43,43,45`, so the dark ground holds where a person actually sees it, and the visible edge is the intended `postio-3s2` hairline at `60,60,62` rather than a card fill. **The light scheme and the switch are measured now too (2026-09-12).** `scripts/appearance.sh` was run whole on this workstation and all seven colour claims are green: light's editing surface, light's reader body and their agreement all read exactly `245,245,248`; dark's three read `43,43,45`; and the control — the same point differing between schemes — reads `245,245,248 -> 43,43,45`, so the scheme reaches the `WebView` rather than the PNGs being stale. FR-073 and FR-074 are therefore measured in both schemes rather than seen in one.

  **One of the three human checks is also answered**: the quote opens *folded* — `▸ Quoted message` under the attribution line, collapsed, in both schemes. That is readable off a still.

  **What actually remains needs hands, and a still cannot stand in for any of it**: the quote opening *when clicked*, the caret sitting above it with typing going where you look, and a scheme change keeping the caret and the undo history. Those are interactions, not pixels
...
- [X] T052 [P] [US5] Assert a failed send stays editable and findable and names what went wrong — FR-066. **Two thirds hold and are asserted** (`storage_suite/drafts.rs`): the draft stays editable with its recipients and text, stays in Drafts, and the reason is kept on the queue row. **The third does not reach any surface** — the reason is built by `drain`, written to `last_error`, and carried up through the engine's report, whose own doc says "the reason the user should see", and then nothing reads it. **Then built.** `OperationQueueRepository::last_failure_for` is the query that was missing — the reason was durable on the queue row and unreachable from a `DraftId`, which is why it could be computed, persisted, carried up the engine's report and still read by nobody. Reopening a failed draft now says `Not sent — <reason>`. Read rather than copied onto the draft: one source of truth, and a second copy is one that can come to disagree with the first. The toast half of #1487 is the follow-on; the draft is the half that cannot decay, and it is where the person has come back to do something about it
...
- [ ] T072 Walk [quickstart.md](./quickstart.md) end to end, including the by-eye checks that no test covers. **Every runnable step walked and green** — section 1 (8 + 38), section 2 (6), section 3 (25 + 42 + 1), section 4 (sanity tier, gtk_suite 275, app_suite 89), and `check.sh`. **One real defect found and fixed in the walking**: the command labelled "the security assertion, which is the one that matters" was `cargo test -p postio-body corpus_quote`, which matches no test — it printed `test result: ok` having run nothing. Every command in the file now carries the count it should select, because `ok. 0 passed` and `ok. 1 passed` look alike at a glance and mean opposite things. **What remains is the by-eye half only**, which is T048 and needs a person at a display. That half has shrunk twice: FR-074's dark value was measured on 2026-09-11, and on 2026-09-12 the light scheme, the scheme switch and the folded quote joined it. Three interactions are left, and no screenshot can answer them — see T048
- [x] T073 Land: `scripts/issue-land.sh --detach` — one pull request, reviewed against spec.md, closing no issue. **PR #1489 was opened deliberately without auto-merge.** The land script always arms it, and T048 had not happened — 38 commits including two ADRs that reverse documented properties should not merge themselves while the one check no test on this machine can perform is outstanding. Opened with `gh pr create` instead, so it is readable and mergeable by hand. **Merged 2026-09-11 as a merge commit** — matching #1453, the sibling feature branch, so the 45 commits and both ADRs survive as history — on the maintainer's instruction and ahead of T048's remaining half. Two defects surfaced in the live run that prompted it, and both turned out to predate this branch: the doubled `To:` in the reader header, where the shared label column and the value both drew the word (#1492), and a resync that held SQLite's write lock past `busy_timeout` and lost a draft the composer was autosaving — the write gate #425 put into `initial.rs` had never been put into `resync.rs` (#1493). Both landed on `main` separately, before this merge
