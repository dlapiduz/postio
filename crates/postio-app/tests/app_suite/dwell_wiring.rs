...
pub fn resting_on_a_message_marks_it_read_and_sweeping_past_does_not() {
    crate::gtk_case(async {
        ...
        let untouched: Vec<MessageId> = page(&database, inbox).await;
        ...
        assert!(
            settle_until(async || is_read(&database, resting).await).await,
...
async fn page(database: &Store, mailbox: postio_model::MailboxId) -> Vec<MessageId> {
    let connection = database.connect().await.expect("a connection");
    MessageRepository::new(&connection)
        .page(&ListQuery {
