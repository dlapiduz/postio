//! The account crate: everything Postio knows about a mail account that
//! is not the protocol it happens to speak.
//!
...
//! where every protocol type is confined — this crate carried that
//! implementation's name (`postio-imap`) until #153, which is also why the
//! `imap` module and feature are still named for it rather than for this
//! crate. ADR 0001 explains why the backend/protocol division is
//! load-bearing rather than tidy; ADR 0006 Q2 is what named the crate for
//! what it actually is.
//!
//! Around them:
//!
...
//! # Live tests
//!
//! `cargo test -p postio-account` never touches the network: every live-server
...
//! cargo test -p postio-account -- --ignored
//! ```
//!
