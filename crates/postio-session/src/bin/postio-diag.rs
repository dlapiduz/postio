...
//!
//! ```text
//! postio-diag census                      # the live store, nothing running
...
//! Against the live store, close the application first: two writers with
//! different rules is the thing ADR 0010 exists to prevent, and a reader
//! that opens the file mid-write measures the recovery, not the store.
//!
...
Reads only. Against the live store, close the application first, or pass
--store with a copy. The key comes from the keyring, or POSTIO_STORE_KEY.";
...
async fn main() -> ExitCode {
    ...
    let path = path.unwrap_or_else(|| {
        eprintln!("postio-diag: reading the live store; close the application first");
    ...
    let report = Report { connection, file };
    let outcome = match command.as_str() {
        ...
        other => {
            eprintln!("postio-diag: no such command {other:?}\n\n{USAGE}");
            return ExitCode::FAILURE;
        }
    };
    match outcome {
...
type Outcome = Result<(), postio_storage::Error>;
...
impl Report {
    ...
    async fn census(&self) -> Outcome {
        ...
        println!("file            {:>12}", bytes(self.file));
        println!(
    ...
    async fn shape(&self) -> Outcome {
        ...
        println!("\nthe file itself: {:.1} MiB", self.file as f64 / 1048576.0);
        Ok(())
    }

    /// What the background lanes still owe.
    async fn pending(&self) -> Outcome {
        println!("bodies, by state:");
        let states = sql::all(
            &self.connection,
        ...
        let unindexed = self
            ...
            .await;
        println!("\nowed:");
        println!("  backfill candidates              {backfill:>10}");
        println!(
            "  re-fetches an older parser owes  {refetch:>10}  (parser v{})",
            postio_model::mime::PARSER_VERSION
        );
        println!("  bodies awaiting the indexer      {unindexed:>10}");
