...
pub fn typing_in_the_box_searches_the_store_and_fills_every_search_surface() {
    crate::gtk_case(async {
        ...
        let answered = settle_until(async || live.outcome().is_some()).await;
        ...
        let offered = settle_until(async || !view.panel().offered().is_empty()).await;
        assert!(
            offered,
        ...
        let focused = settle_until(async || view.preview().focused().is_some()).await;
        assert!(
            focused,
        ...
        let correspondents = settle_until(async || !finder.matched_contacts().is_empty()).await;
        assert!(
            correspondents,
