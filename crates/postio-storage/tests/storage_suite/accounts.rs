...
async fn an_account_round_trips_through_the_database() {
    ...
    let id = accounts.create(&mut account).await.expect("create");
...
#[tokio::test]
async fn an_account_created_with_identities_gets_them_all_persisted() {
    let database = test_support::memory().await;
    let connection = database.connect().await.expect("checkout");
    let accounts = AccountRepository::new(&connection);
    ...
    let stored = accounts.get(id).await.expect("get").expect("the account");
...
#[tokio::test]
async fn reading_an_account_that_is_not_there_is_none_rather_than_an_error() {
    let database = test_support::memory().await;
    let connection = database.connect().await.expect("checkout");
    let accounts = AccountRepository::new(&connection);
...
#[tokio::test]
async fn listing_returns_accounts_in_a_stable_order_and_can_filter_to_enabled() {
    let database = test_support::memory().await;
    let connection = database.connect().await.expect("checkout");
    let accounts = AccountRepository::new(&connection);

    let mut first = an_account();
    accounts.create(&mut first).await.expect("create");
    let mut second = Account::new(
        "Fastmail",
        EmailAddress::new(None::<String>, "b@example.com"),
    );
    second.enabled = false;
    accounts.create(&mut second).await.expect("create");

    let all = accounts.list().await.expect("list");
    assert_eq!(all.len(), 2);
...
#[tokio::test]
async fn updating_an_account_changes_its_row() {
    let database = test_support::memory().await;
    let connection = database.connect().await.expect("checkout");
    let accounts = AccountRepository::new(&connection);
...
#[tokio::test]
async fn updating_reconciles_the_identity_list() {
    let database = test_support::memory().await;
    let connection = database.connect().await.expect("checkout");
    let accounts = AccountRepository::new(&connection);
...
#[tokio::test]
async fn updating_an_unpersisted_account_is_an_error_rather_than_a_silent_no_op() {
    let database = test_support::memory().await;
    let connection = database.connect().await.expect("checkout");
    let accounts = AccountRepository::new(&connection);

    assert!(accounts.update(&mut an_account()).await.is_err());
}
...
#[tokio::test]
async fn identities_can_be_managed_without_rewriting_the_account() {
    let database = test_support::memory().await;
    let connection = database.connect().await.expect("checkout");
    let accounts = AccountRepository::new(&connection);
    ...
    let mut account = an_account();
    let account_id = accounts.create(&mut account).await.expect("create");
    ...
    identities.update(&identity).await.expect("update");
    assert_eq!(
        identities
            .get(id)
            .await
    ...
    assert!(identities.get(id).await.expect("get").is_none());
    assert!(
        !identities.delete(id).await.expect("delete again"),
        "deleting what is gone is false, not an error"
    );
}

#[tokio::test]
async fn an_account_has_at_most_one_default_identity() {
    let database = test_support::memory().await;
    let connection = database.connect().await.expect("checkout");
    let accounts = AccountRepository::new(&connection);
    let identities = IdentityRepository::new(&connection);
...
#[tokio::test]
async fn deleting_an_account_takes_everything_that_hangs_off_it() {
    let database = test_support::memory().await;
    let connection = database.connect().await.expect("checkout");
    let accounts = AccountRepository::new(&connection);
    ...
    assert!(accounts.delete(id).await.expect("delete"));
    ...
    assert!(
        !accounts.delete(id).await.expect("delete again"),
        "deleting what is gone is false, not an error"
    );
}
...
#[tokio::test]
async fn set_enabled_flips_only_that_column() {
    let database = test_support::memory().await;
    let connection = database.connect().await.expect("checkout");
    let accounts = AccountRepository::new(&connection);
    ...
    let id = accounts.create(&mut account).await.expect("create");
    assert!(accounts.get(id).await.unwrap().unwrap().enabled);

    assert!(accounts.set_enabled(id, false).await.expect("disable"));
    let stored = accounts.get(id).await.expect("get").expect("still there");
    assert!(!stored.enabled);
    assert_eq!(
    ...
    assert!(accounts.set_enabled(id, true).await.expect("re-enable"));
    assert!(accounts.get(id).await.unwrap().unwrap().enabled);
}

#[tokio::test]
async fn set_enabled_on_a_missing_account_is_false_not_an_error() {
    let database = test_support::memory().await;
    let connection = database.connect().await.expect("checkout");
    let accounts = AccountRepository::new(&connection);
...
#[tokio::test]
async fn marking_for_deletion_is_reversible_until_it_is_reaped() {
    let database = test_support::memory().await;
    let connection = database.connect().await.expect("checkout");
    let accounts = AccountRepository::new(&connection);
    ...
    let mut account = an_account();
    let id = accounts.create(&mut account).await.expect("create");
    assert!(!accounts.get(id).await.unwrap().unwrap().pending_deletion);

    assert!(accounts.mark_pending_deletion(id).await.expect("mark"));
    assert!(
        accounts.get(id).await.unwrap().unwrap().pending_deletion,
        "marked, but not yet actually deleted"
    );

    assert!(accounts.restore(id).await.expect("restore"));
    assert!(
        !accounts.get(id).await.unwrap().unwrap().pending_deletion,
...
#[tokio::test]
async fn a_marked_account_is_excluded_from_the_enabled_list_even_before_it_is_reaped() {
    let database = test_support::memory().await;
    let connection = database.connect().await.expect("checkout");
    let accounts = AccountRepository::new(&connection);

    let mut kept = an_account();
    accounts.create(&mut kept).await.expect("create");
...
#[tokio::test]
async fn reaping_deletes_every_marked_account_and_leaves_the_rest() {
    let database = test_support::memory().await;
    let connection = database.connect().await.expect("checkout");
    let accounts = AccountRepository::new(&connection);
    ...
    assert!(accounts.get(leaving_id).await.expect("get").is_none());
    assert!(accounts.get(kept.id).await.expect("get").is_some());
}

#[tokio::test]
async fn reaping_cascades_exactly_like_an_ordinary_delete() {
    let database = test_support::memory().await;
    let connection = database.connect().await.expect("checkout");
    let accounts = AccountRepository::new(&connection);
    ...
    accounts.mark_pending_deletion(id).await.expect("mark");
...
#[tokio::test]
async fn reaping_with_nothing_marked_deletes_nothing() {
    let database = test_support::memory().await;
    let connection = database.connect().await.expect("checkout");
    let accounts = AccountRepository::new(&connection);
...
#[tokio::test]
async fn an_accounts_named_signatures_round_trip_and_arrive_with_it() {
...
#[tokio::test]
async fn two_accounts_never_see_each_others_signatures() {
    let database = test_support::memory().await;
    let connection = database.connect().await.expect("checkout");
    let accounts = AccountRepository::new(&connection);
    let mut mine = an_account();
    accounts.create(&mut mine).await.expect("create");
    let mut theirs = Account::new(
        "Other",
        EmailAddress::new(None::<String>, "other@example.net"),
    );
    accounts.create(&mut theirs).await.expect("create");
    ...
    let loaded = accounts.get(mine.id).await.expect("get").expect("there");
    assert_eq!(loaded.signatures.len(), 1);
    assert_eq!(loaded.signatures[0].text, "Lena");
}

#[tokio::test]
async fn oauth_composition_data_round_trips_and_stays_optional() {
...
#[tokio::test]
async fn the_refresh_grant_lifetime_round_trips_and_stays_optional() {
...
#[tokio::test]
async fn the_backend_choice_round_trips_and_defaults_to_imap() {
...
async fn two_signatures_in_one_account_cannot_share_a_name() {
    ...
    let loaded = accounts.get(account.id).await.expect("get").expect("there");
    assert_eq!(loaded.signatures.len(), 1);
    assert_eq!(loaded.signatures[0].text, "— Lena");
}

#[tokio::test]
async fn the_same_name_in_a_different_account_is_fine() {
...
#[tokio::test]
async fn deleting_the_default_signature_leaves_no_dangling_reference() {
    ...
    assert!(signatures.delete(work.id).await.expect("delete"));
...
#[tokio::test]
async fn at_most_one_account_is_the_default_and_setting_it_moves_the_marker() {
    let database = test_support::memory().await;
    let connection = database.connect().await.expect("checkout");
    let accounts = AccountRepository::new(&connection);

    let mut first = an_account();
    let first_id = accounts.create(&mut first).await.expect("create");
...
#[tokio::test]
async fn a_new_account_is_not_the_default_until_somebody_says_so() {
    let database = test_support::memory().await;
    let connection = database.connect().await.expect("checkout");
    let accounts = AccountRepository::new(&connection);
...
#[tokio::test]
async fn set_default_flips_only_that_column() {
    let database = test_support::memory().await;
    let connection = database.connect().await.expect("checkout");
    let accounts = AccountRepository::new(&connection);
    ...
    let mut account = an_account();
    let id = accounts.create(&mut account).await.expect("create");
    let before = accounts.get(id).await.expect("get").expect("the account");

    accounts.set_default(id).await.expect("mark it");

    let after = accounts.get(id).await.expect("get").expect("the account");
...
#[tokio::test]
async fn set_default_on_a_missing_account_leaves_the_marker_where_it_was() {
    let database = test_support::memory().await;
    let connection = database.connect().await.expect("checkout");
    let accounts = AccountRepository::new(&connection);
    ...
    assert!(
        accounts.set_default(missing).await.is_err(),
...
#[tokio::test]
async fn reaping_a_removed_account_takes_its_default_marker_with_it() {
    let database = test_support::memory().await;
    let connection = database.connect().await.expect("checkout");
    let accounts = AccountRepository::new(&connection);
    ...
    let id = accounts.create(&mut account).await.expect("create");
    accounts.set_default(id).await.expect("mark it");
    accounts.mark_pending_deletion(id).await.expect("remove it");
