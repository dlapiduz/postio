...
public final class KeyWindowTracker {
    ...
    public static func tag(_ window: NSWindow, as role: KeyWindow) {
        window.identifier = NSUserInterfaceItemIdentifier(identifier(for: role))
    }
    ...
    static func role(of window: NSWindow?) -> KeyWindow {
        ...
        return KeyWindow.allCases.first { identifier(for: $0) == raw } ?? .main
    }
