...
async fn an_account(connection: &Connection, address: &str) -> AccountId {
...
#[tokio::test]
async fn a_role_maps_to_one_path_per_account_and_a_second_choice_replaces_the_first() {
    let database = test_support::memory().await;
    let connection = database.connect().await.expect("checkout");
    let account = an_account(&connection, "ada@example.com").await;
    ...
    assert_eq!(
        roles.for_account(account).await.expect("the map"),
        vec![
...
#[tokio::test]
async fn an_accounts_map_is_its_own_and_dies_with_it() {
    let database = test_support::memory().await;
    let connection = database.connect().await.expect("checkout");
    let icloud = an_account(&connection, "ada@example.com").await;
    let other = an_account(&connection, "ada@example.net").await;
    let roles = MailboxRoleRepository::new(&connection);
    ...
    assert!(
        roles.for_account(other).await.expect("the map").is_empty(),
    ...
    assert!(
        roles.for_account(icloud).await.expect("the map").is_empty(),
...
#[tokio::test]
async fn a_refusal_is_recorded_per_account_and_role_with_the_servers_own_words() {
    let database = test_support::memory().await;
    let connection = database.connect().await.expect("checkout");
    let ada = an_account(&connection, "ada@example.com").await;
    let grace = an_account(&connection, "grace@example.net").await;
    let roles = MailboxRoleRepository::new(&connection);
    ...
    assert_eq!(
        roles.refusals(ada).await.expect("refusals"),
        vec![(MailboxRole::Junk, "Permission denied".to_owned())]
    );
    assert_eq!(
        roles.refusals(grace).await.expect("refusals"),
        vec![(MailboxRole::Junk, "Mailbox already exists".to_owned())]
    );
}

#[tokio::test]
async fn refusing_the_same_role_twice_keeps_the_current_reason_not_a_second_row() {
    ...
    assert_eq!(
        roles.refusals(account).await.expect("refusals"),
        vec![(MailboxRole::Trash, "Quota exceeded".to_owned())]
    );
}

#[tokio::test]
async fn mapping_a_role_by_hand_clears_the_refusal_that_was_standing_against_it() {
    ...
    assert!(
        roles.refusals(account).await.expect("refusals").is_empty(),
        "the refusal outlived the question it was answering"
    );
}

#[tokio::test]
async fn a_refusal_goes_with_its_account() {
    let database = test_support::memory().await;
    let connection = database.connect().await.expect("checkout");
    let account = an_account(&connection, "ada@example.com").await;
    let roles = MailboxRoleRepository::new(&connection);
