...
/// and the way out.
///
/// Three counts, because there are two different ways for the column to be
...
///
/// `resident < total` is an **absence**: messages exist that are not here, and
/// the line says `n of m messages here` — a column that showed three of six
/// and called it the thread would be lying about the one thing the user came
/// to see.
///
/// `shown < resident` is a **filter**: the user turned on "unread only" and
/// the rest are a keystroke away. Reporting that as `3 of 8 messages here`
/// says five messages are missing when none are, which is the same lie from
/// the other direction (#436).
pub fn summary(
    shown: usize,
    resident: usize,
    total: u32,
    ...
    // The badge on the list row can lag what the column holds, so the
    // conversation is never smaller than what is already in hand.
...
mod tests {
    ...
    fn filtering_to_unread_is_not_the_same_as_messages_missing() {
        ...
        // them because "unread only" is on. Saying "3 of 8 messages here"
        // would report the filter as an absence -- five messages this
        // machine does not have -- which is the one thing the `n of m`
        // wording exists to report honestly (#436).
        assert_eq!(
    ...
    fn a_filtered_column_that_is_also_incomplete_still_counts_the_whole_thread() {
        // Both at once: six of eight are here, and "unread only" hides three
        // of those six. The denominator stays the conversation.
        assert_eq!(
