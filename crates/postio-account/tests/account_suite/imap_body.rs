...
use postio_account::backend::{BackendError, BodyPart, CountingSink, UidSet, VecSink};
...
async fn live_server_streams_a_real_body() {
    ...
    let fetched = fetch_part(
        &pool,
        "INBOX",
        &postio_account::backend::identity::remote_id(message.uid_validity, message.uid),
        &BodyPart::Whole,
        &mut sink,
        Priority::Interactive,
        &CancelToken::new(),
