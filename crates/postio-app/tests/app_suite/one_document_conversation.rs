...
pub fn a_single_message_conversation_still_offers_its_verbs() {
    crate::gtk_case(async {
        ...
        threaded_message(&database, &seat, 0, "message 0", &body_of(0), true).await;
        ...
        window.open_conversation(&cursor);
        assert!(
            settle_until(async || window.conversation().thread_document().is_some()).await,
