...
async fn labels_of(database: &Store, message: MessageId) -> Vec<LabelId> {
...
pub fn a_label_command_puts_a_label_on_the_message_it_names() {
    crate::gtk_case(async {
        ...
        assert!(
            labels_of(&database, first).await.is_empty(),
        ...
        assert!(
            settle_until(async || labels_of(&database, first).await == vec![work]).await,
            "the command never reached a verb that writes: the message carries \
             {:?}",
            labels_of(&database, first).await
        );

        // ── and `u` takes it back, which the registry promises ───────────────
        window.act(postio_core::Command::Undo);
        assert!(
            settle_until(async || labels_of(&database, first).await.is_empty()).await,
