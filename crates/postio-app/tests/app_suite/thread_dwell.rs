...
async fn is_read(database: &Store, message: MessageId) -> bool {
...
pub fn resting_inside_a_conversation_reads_each_message_as_focus_reaches_it() {
    crate::gtk_case(async {
        ...
        let untouched = members_of(new_thread().await, 100).await;
        let members = members_of(new_thread().await, 0).await;
        ...
        assert!(
            {
                ...
                for id in &untouched {
                    all_unread &= !is_read(&database, *id).await;
                }
                all_unread
            },
        ...
        assert!(
            settle_until(async || is_read(&database, newest).await).await,
            "resting on the message the pane opened on never marked it read"
        );
        assert!(
            !is_read(&database, members[0]).await && !is_read(&database, members[1]).await,
        ...
        for member in [members[0], members[1]] {
            ...
            assert!(
                settle_until(async || is_read(&database, member).await).await,
        ...
        assert!(
            settle_until(async || list.cursor_row().is_some_and(|row| row.seen)).await,
