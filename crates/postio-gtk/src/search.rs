...
// rate. Re-exported so every reference in this crate still reads.
/// What to run when a debounced query comes due.
///
/// Stays here with `Live`, the only thing that holds one: it is a callback
/// into a GTK widget's own bookkeeping, not part of the pacing rule.
type RunHandler = Box<dyn Fn(&ParsedQuery, u64)>;

// The readout's label widths stay here: they size a `gtk::Label` in
// characters, which is this toolkit's unit and this frontend's problem. The
// *wording* they have to fit is `postio_ui::search::readout`'s, and
// `a_readout_fits_its_label` below is what keeps the two in step.
...
pub use postio_ui::search::{
    Backspace, Chip, DEBOUNCE, Outcome, Pacer, backspace, chips, readout, spoken, spoken_readout,
};
