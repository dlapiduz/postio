...
//! read-write, because `Store::open` migrates and `ensure_search_index`
...
pub fn a_real_account_answers_a_real_query() {
    crate::gtk_case(async {
        ...
        assert!(
            settle_until(async || live.outcome().is_some()).await,
        ...
        assert!(
            settle_until(async || !view.panel().offered().is_empty()).await,
            "the refine column offered nothing for {} hits",
            outcome.hits
        );
        assert!(
            settle_until(async || view.preview().focused().is_some()).await,
        ...
        assert!(
            settle_until(async || !finder.matched_contacts().is_empty()).await,
