...
use postio_account::backend::UidSet;
...
async fn a_cancelled_token_stops_before_any_round_trip() {
    ...
    assert!(matches!(
        error,
        postio_account::backend::BackendError::Cancelled
    ));
    assert!(connector.log().tls.is_empty());
}
...
async fn a_line_io_imap_could_not_decode_forces_a_full_resync_not_a_silent_success() {
    ...
    let postio_account::backend::BackendError::ResyncIntegrityLost { mailbox, skipped } = &error
    else {
...
async fn the_same_undecodable_line_is_tolerated_outside_a_changedsince_fetch() {
    ...
    let _exclusive = postio_account::imap::skip_counter_exclusive_measurement().await;
