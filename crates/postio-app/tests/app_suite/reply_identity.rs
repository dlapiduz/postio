...
async fn account_with_identity(database: &Store, display_name: &str, address: &str) -> Account {
...
pub fn a_reply_to_a_message_in_a_second_account_uses_that_accounts_identity() {
    crate::gtk_case(async {
        ...
        let account_a = account_with_identity(&database, "Personal", "ada@example.com").await;
        let account_b = account_with_identity(&database, "Work", "grace@example.com").await;
        assert_ne!(
        ...
        assert!(
            settle_until(async || list.cursor_id() == Some(message_b)).await,
