...
Usage: scripts/tests/test-issue-land-auto-merge.py
...
if [ "$1" = "pr" ] && [ "$2" = "merge" ]; then
    for arg in "$@"; do
        case "$arg" in
            --auto) echo "auto-merge enabled for pull request #1"; exit 0 ;;
        esac
    done
    for arg in "$@"; do
        case "$arg" in
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(dir=REPO_ROOT) as directory:
        ...
        if "auto-merge" not in result.stdout:
            FAILURES.append(f"the script did not say auto-merge is armed:\n{result.stdout}")
        if "merged." in result.stdout:
            FAILURES.append(f"the script claimed a merge that has not happened:\n{result.stdout}")
        remote_branches = git("ls-remote", "--heads", "origin", cwd=root).stdout.decode()
        if "issue-1-x" not in remote_branches:
            FAILURES.append("the remote branch was deleted; GitHub deletes it on merge, and until then it is the PR")
        if "--resume" not in result.stdout:
            FAILURES.append(f"the script did not say how to come back if CI fails:\n{result.stdout}")
    if FAILURES:
        print(f"{len(FAILURES)} case(s) failed:\n", file=sys.stderr)
        for failure in FAILURES:
            print(f"- {failure}\n", file=sys.stderr)
        return 1
    print(f"issue-land auto-merge check passed ({channel}).")
    return 0
