...
async fn plant(database: &Store, account: postio_model::AccountId) -> postio_model::MessageId {
...
pub fn a_unified_search_reaches_every_account() {
    crate::gtk_case(async {
        ...
        let answered = settle_until(async || hits(&window).is_some()).await;
        ...
        assert!(
            settle_until(async || hits(&window) == Some(0)).await,
