...
- [ ] T056 [US5] Implement the one-in-the-pane, many-detached rule in `crates/postio-gtk/src/shell.rs` and `composer.rs` — FR-010, FR-011, FR-013, FR-014. **Decided, not built.** [ADR 0034](../../docs/decisions/0034-one-composer-in-the-pane-many-in-windows.md) records the decision, what replaces the "same widget, reparented" safety property (the draft is the record — ADR 0004 — so moving one between surfaces is a save and a resume), what is knowingly lost (the undo history, the caret), and the three places the build touches. The third of those has no answer yet and should be settled first: with several composers open, command dispatch reaches `window.composer()` and `CommandId::Send` has to reach the one the keyboard is in — the focused window's, not the pane's. `Window::open_draft` exists with today's semantics so the acceptance compiles and fails rather than not naming anything. **Blocked on a routing decision, deliberately.** `Window::connect_command` is a broadcast with no notion of which surface resolved the key, and a composer subscribes on mount and never unsubscribes — so two open composers means `Send` sends both drafts. A focus test inside `dispatch` is the obvious guard and is a trap: windows are not reliably active headless, so it would disable the composer across most of the suite while reading as correct. The routing has to be *carried* — `connect_command` saying which surface it came from — which is a signature change across every subscriber and belongs at the front of the build. Until then the state half must not be built alone: a `Window` holding several composers while dispatch still broadcasts is worse than today, because it would send drafts nobody asked to send
