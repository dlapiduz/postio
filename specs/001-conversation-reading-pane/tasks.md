...
- [X] T114 Merge `feature/conversation-reading-pane` to `main` when whole. **Done: merged 2026-09-10 as [#1453](https://github.com/dlapiduz/postio/pull/1453)**, and the branch is gone from the remote. It was left unmarked, so this file claimed pending work that had shipped — checked on 2026-09-12 against `origin/main`, where `specs/001-conversation-reading-pane/` and the pane itself both live.

  Of the three things the note below said it waited on, two closed before the merge and one did not block it: **#1426** (the stacked pane) and **#1427** (the recipients regression) are CLOSED; **#1424**, ADR 0032's screen-reader gate, is still OPEN and is tracked as its own issue rather than as a condition on this merge. The original note, for the record:

  > **Rebasing is current** -- `origin/main` merged in on 2026-09-09 (merge, not rebase: `/initiative` says the branch is shared), 0 behind after. A traceability map of the spec's FRs is on the epic (#1405). **Now waits on three things the maintainer's 2026-09-09 decisions created**, none of which existed this morning: **#1424** ADR 0032's screen-reader gate, accepted-without and owed after the fact; **#1426** the stacked pane, which the application no longer builds and which is what would be restored if #1424 goes badly; and **#1427** the recipients regression the default flip exposed (landing). T105 becomes correct as written the day #1426 says retire
