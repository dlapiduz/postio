...
// The section model moved to `postio_ui::settings` for ADR 0029: both
// frontends navigate the same six sections of the same file, and a second
// copy of "which line does `[sync]` start on" is a second answer waiting to
// disagree. `Section::label` is still what this panel shows — the bracketed
// table name, which a text view over the real file has to agree with — while
// a structured pane shows `Section::title`.
pub use postio_ui::settings::{Section, find_section, section_at_line};
